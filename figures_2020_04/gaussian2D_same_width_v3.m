...
if sum(diff(regionX)>0.010001) ==0 
   line([regionX(1),regionX(end)],[threshold,threshold], 'Color',region_color,'LineWidth',6)
   hold off
else
   midPoint = find(diff(regionX)>0.01001);
   line([regionX(1),regionX(midPoint)],[threshold,threshold], 'Color',region_color,'LineWidth',6)
   hold on
   line([regionX(midPoint+1),regionX(end)],[threshold,threshold], 'Color',region_color,'LineWidth',6)
   hold off
end
hold off
ylabel('U')
...
hold on
scatter(x(max_i),threshold,'MarkerEdgeColor',[0 .7 .7],'MarkerFaceColor',[0 .7 .7],'LineWidth',1.5)
hold off
